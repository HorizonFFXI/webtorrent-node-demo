...
class Storage {
  constructor(chunkLength, opts = {}) {
    ...
    this.name =
      opts.name || path.join("fs-chunk-store", randombytes(20).toString("hex"));
    ...
    if (opts.files) {
      ...
      this.files = opts.files.map((file, i, files) => {
        ...
        if (this.path) {
          newPath = this.addUID
            ? path.resolve(path.join(this.path, this.name, file.path))
            : path.resolve(path.join(this.path, file.path));
        }
    ...
    if (this.length !== Infinity) {
      ...
      this.files.forEach((file) => {
        ...
        for (let p = firstChunk; p <= lastChunk; ++p) {
          ...
          const to =
            fileEnd > chunkEnd ? this.chunkLength : fileEnd - chunkStart;
  ...
  destroy(cb) {
    this.close(() => {
      if (this.addUID && this.path) {
        fs.rm(
          path.resolve(path.join(this.path, this.name)),
          { recursive: true, maxBusyTries: 10 },
          cb
        );
      } else {
        const tasks = this.files.map((file) => {
